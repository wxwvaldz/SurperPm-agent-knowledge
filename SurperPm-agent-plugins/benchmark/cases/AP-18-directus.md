---
id: AP-18
title: Collection compliance review
level: AP
track: B
target_repo_type: node-vue-app
target_repo: ap-directus
...
oracle: |
  L2+ application task: Add a compliance-review metadata path for collections/items, expose it through API/admin UI list filtering, and respect existing permission and schema conventions.
...
# AP-18 Collection compliance review

## Goal

在 `Headless CMS` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a compliance-review metadata path for collections/items, expose it through API/admin UI list filtering, and respect existing permission and schema conventions.
