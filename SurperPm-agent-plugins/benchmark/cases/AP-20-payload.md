---
id: AP-20
title: Collection publish readiness checklist
level: AP
track: B
target_repo_type: next-app
target_repo: ap-payload
...
oracle: |
  L2+ application task: Add a publish-readiness metadata field for collection documents, carry it through admin UI and API reads, and ensure draft/publish flows still behave correctly.
...
# AP-20 Collection publish readiness checklist

## Goal

在 `CMS` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a publish-readiness metadata field for collection documents, carry it through admin UI and API reads, and ensure draft/publish flows still behave correctly.
