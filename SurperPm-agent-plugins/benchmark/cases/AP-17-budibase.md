---
id: AP-17
title: App publish approval gate
level: AP
track: B
target_repo_type: fullstack-js
target_repo: ap-budibase
...
oracle: |
  L2+ application task: Add an approval state before an internal app can be published, wire it through app metadata, API/service validation, builder UI, and relevant tests or verification notes.
...
# AP-17 App publish approval gate

## Goal

在 `Internal tools` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an approval state before an internal app can be published, wire it through app metadata, API/service validation, builder UI, and relevant tests or verification notes.
