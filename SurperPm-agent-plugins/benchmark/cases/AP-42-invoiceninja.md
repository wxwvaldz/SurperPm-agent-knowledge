---
id: AP-42
title: Client payment-risk marker
level: AP
track: B
target_repo_type: laravel-app
target_repo: ap-invoiceninja
...
oracle: |
  L2+ application task: Add a payment-risk marker for clients or invoices, propagate it through API/list/detail UI, and avoid changing payment gateway behavior.
...
# AP-42 Client payment-risk marker

## Goal

在 `Invoicing` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a payment-risk marker for clients or invoices, propagate it through API/list/detail UI, and avoid changing payment gateway behavior.
