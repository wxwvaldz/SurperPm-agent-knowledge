---
id: AP-34
title: Asset decommission review workflow
level: AP
track: B
target_repo_type: django-app
target_repo: ap-netbox
...
oracle: |
  L2+ application task: Add a decommission-review state for selected infrastructure objects, surface it in list/detail/API views, and preserve existing status semantics.
...
# AP-34 Asset decommission review workflow

## Goal

在 `Infrastructure inventory` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a decommission-review state for selected infrastructure objects, surface it in list/detail/API views, and preserve existing status semantics.
