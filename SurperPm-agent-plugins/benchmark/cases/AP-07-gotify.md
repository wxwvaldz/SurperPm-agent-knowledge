---
id: AP-07
title: Notification snooze metadata
level: AP
track: B
target_repo_type: go-web-app
target_repo: ap-gotify
...
oracle: |
  L2+ application task: Add a snooze-until metadata path for messages or applications, propagate it through REST/WebSocket/UI status handling, and make default message lists hide snoozed items where appropriate.
...
# AP-07 Notification snooze metadata

## Goal

在 `Notification server` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a snooze-until metadata path for messages or applications, propagate it through REST/WebSocket/UI status handling, and make default message lists hide snoozed items where appropriate.
