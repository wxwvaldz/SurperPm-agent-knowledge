---
id: AP-47
title: Tool workflow owner approval
level: AP
track: B
target_repo_type: node-react-app
target_repo: ap-tooljet
...
oracle: |
  L2+ application task: Add owner approval metadata for ToolJet apps or workflows, expose it in builder/list UI and API validation, and preserve existing permissions.
...
# AP-47 Tool workflow owner approval

## Goal

在 `Internal tools` 应用仓中完成一个 L2+ 级别的跨模块需求：Add owner approval metadata for ToolJet apps or workflows, expose it in builder/list UI and API validation, and preserve existing permissions.
