---
id: AP-12
title: Survey response triage queue
level: AP
track: B
target_repo_type: next-app
target_repo: ap-formbricks
...
oracle: |
  L2+ application task: Add a triage status for survey responses, expose it in response APIs and dashboard filters, and include UI affordances for unresolved/high-priority feedback.
...
# AP-12 Survey response triage queue

## Goal

在 `Survey platform` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a triage status for survey responses, expose it in response APIs and dashboard filters, and include UI affordances for unresolved/high-priority feedback.
