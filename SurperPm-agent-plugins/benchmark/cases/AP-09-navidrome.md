---
id: AP-09
title: Playlist curation review flow
level: AP
track: B
target_repo_type: go-react-app
target_repo: ap-navidrome
...
oracle: |
  L2+ application task: Add playlist curation status metadata that appears in playlist list/detail views, supports a reviewed/unreviewed filter, and preserves existing music streaming behavior.
...
# AP-09 Playlist curation review flow

## Goal

在 `Music server` 应用仓中完成一个 L2+ 级别的跨模块需求：Add playlist curation status metadata that appears in playlist list/detail views, supports a reviewed/unreviewed filter, and preserves existing music streaming behavior.
