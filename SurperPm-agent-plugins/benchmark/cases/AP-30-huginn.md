---
id: AP-30
title: Agent run safety review
level: AP
track: B
target_repo_type: rails-app
target_repo: ap-huginn
...
oracle: |
  L2+ application task: Add a safety-review state for automation agents or recent runs, surface it in agent/run UI and service responses, and keep scheduling behavior intact.
...
# AP-30 Agent run safety review

## Goal

在 `Automation agents` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a safety-review state for automation agents or recent runs, surface it in agent/run UI and service responses, and keep scheduling behavior intact.
