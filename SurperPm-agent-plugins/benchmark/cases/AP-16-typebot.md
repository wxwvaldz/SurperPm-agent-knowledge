---
id: AP-16
title: Bot conversation handoff review
level: AP
track: B
target_repo_type: fullstack-ts
target_repo: ap-typebot
...
oracle: |
  L2+ application task: Add a human-handoff review marker for bot conversation results, propagate it through builder/runtime data surfaces, and add dashboard filtering for unresolved handoffs.
...
# AP-16 Bot conversation handoff review

## Goal

在 `Chatbot builder` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a human-handoff review marker for bot conversation results, propagate it through builder/runtime data surfaces, and add dashboard filtering for unresolved handoffs.
