---
id: AP-23
title: Notification template rollout status
level: AP
track: B
...
oracle: |
  L2+ application task: Add a staged-rollout status for notification templates, expose it in API/dashboard surfaces, and make default send paths respect non-active templates.
...
# AP-23 Notification template rollout status

## Goal

在 `Notification platform` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a staged-rollout status for notification templates, expose it in API/dashboard surfaces, and make default send paths respect non-active templates.
