---
id: AP-37
title: Page publication readiness state
level: AP
track: B
target_repo_type: django-app
target_repo: ap-django-cms
...
oracle: |
  L2+ application task: Add publication readiness metadata for CMS pages, surface it in admin/list views, and preserve existing draft/publish permission behavior.
...
# AP-37 Page publication readiness state

## Goal

在 `CMS` 应用仓中完成一个 L2+ 级别的跨模块需求：Add publication readiness metadata for CMS pages, surface it in admin/list views, and preserve existing draft/publish permission behavior.
