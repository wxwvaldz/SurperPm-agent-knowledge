---
id: AP-41
title: Invoice approval exception workflow
level: AP
track: B
target_repo_type: laravel-app
target_repo: ap-akaunting
...
oracle: |
  L2+ application task: Add an approval-exception state for invoices or bills, expose it in list/detail UI and service validation, and preserve accounting totals.
...
# AP-41 Invoice approval exception workflow

## Goal

在 `Accounting` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an approval-exception state for invoices or bills, expose it in list/detail UI and service validation, and preserve accounting totals.
