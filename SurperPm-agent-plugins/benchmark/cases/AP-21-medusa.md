---
id: AP-21
title: Order fulfillment exception queue
level: AP
track: B
target_repo_type: node-app
target_repo: ap-medusa
...
oracle: |
  L2+ application task: Add fulfillment-exception metadata for orders, propagate it through services/API/admin display, and keep payment/order state transitions safe.
...
# AP-21 Order fulfillment exception queue

## Goal

在 `Commerce backend` 应用仓中完成一个 L2+ 级别的跨模块需求：Add fulfillment-exception metadata for orders, propagate it through services/API/admin display, and keep payment/order state transitions safe.
