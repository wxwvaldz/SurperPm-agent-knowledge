---
id: AP-26
title: Conversation SLA breach triage
level: AP
track: B
target_repo_type: rails-vue-app
target_repo: ap-chatwoot
...
oracle: |
  L2+ application task: Add an SLA breach triage state for conversations, propagate it through inbox APIs and UI badges/filters, and keep assignment/status behavior compatible.
...
# AP-26 Conversation SLA breach triage

## Goal

在 `Support inbox` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an SLA breach triage state for conversations, propagate it through inbox APIs and UI badges/filters, and keep assignment/status behavior compatible.
