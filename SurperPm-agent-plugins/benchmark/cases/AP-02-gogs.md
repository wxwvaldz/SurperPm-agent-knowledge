---
id: AP-02
title: Repository visibility review workflow
level: AP
track: B
...
oracle: |
  L2+ application task: Introduce a repository visibility review state that distinguishes public/internal/private review needs, exposes it through API responses, and adds a list filter plus UI indicator without weakening existing access control.
...
# AP-02 Repository visibility review workflow

## Goal

在 `Repository hosting` 应用仓中完成一个 L2+ 级别的跨模块需求：Introduce a repository visibility review state that distinguishes public/internal/private review needs, exposes it through API responses, and adds a list filter plus UI indicator without weakening existing access control.
