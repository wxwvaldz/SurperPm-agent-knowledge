---
id: AP-32
title: Check maintenance-window awareness
level: AP
track: B
target_repo_type: django-app
target_repo: ap-healthchecks
...
oracle: |
  L2+ application task: Add maintenance-window metadata for checks so scheduled downtime is visible in API/UI and default alerting or status views account for it.
...
# AP-32 Check maintenance-window awareness

## Goal

在 `Cron monitoring` 应用仓中完成一个 L2+ 级别的跨模块需求：Add maintenance-window metadata for checks so scheduled downtime is visible in API/UI and default alerting or status views account for it.
