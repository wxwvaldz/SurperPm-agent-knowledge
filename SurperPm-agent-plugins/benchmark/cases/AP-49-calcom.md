---
id: AP-49
title: Booking policy exception review
level: AP
track: B
target_repo_type: next-app
target_repo: ap-calcom
...
oracle: |
  L2+ application task: Add policy-exception review metadata for bookings/event types, expose it in scheduling/admin surfaces, and preserve availability calculations.
...
# AP-49 Booking policy exception review

## Goal

在 `Scheduling` 应用仓中完成一个 L2+ 级别的跨模块需求：Add policy-exception review metadata for bookings/event types, expose it in scheduling/admin surfaces, and preserve availability calculations.
