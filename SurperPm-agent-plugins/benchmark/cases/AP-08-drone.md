---
id: AP-08
title: Pipeline visibility propagation
level: AP
track: B
target_repo_type: go-web-app
target_repo: ap-drone
...
oracle: |
  L2+ application task: Add execution visibility metadata for pipelines/executions and carry it through store types, API/swagger, execution list/detail UI, and verification without dependency churn.
...
# AP-08 Pipeline visibility propagation

## Goal

在 `CI dashboard` 应用仓中完成一个 L2+ 级别的跨模块需求：Add execution visibility metadata for pipelines/executions and carry it through store types, API/swagger, execution list/detail UI, and verification without dependency churn.
