---
id: AP-36
title: Watch change importance triage
level: AP
track: B
target_repo_type: python-web-app
target_repo: ap-changedetection
...
oracle: |
  L2+ application task: Add an importance triage status for watched changes, expose unresolved important changes in list/detail UI and backend state, and keep notification behavior safe.
...
# AP-36 Watch change importance triage

## Goal

在 `Change monitoring` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an importance triage status for watched changes, expose unresolved important changes in list/detail UI and backend state, and keep notification behavior safe.
