---
id: AP-28
title: Report escalation audit trail
level: AP
track: B
target_repo_type: rails-react-app
target_repo: ap-mastodon
...
oracle: |
  L2+ application task: Add an escalation audit marker for moderation reports, expose it in admin report views and API/model paths, and preserve federation safety behavior.
...
# AP-28 Report escalation audit trail

## Goal

在 `Social network` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an escalation audit marker for moderation reports, expose it in admin report views and API/model paths, and preserve federation safety behavior.
