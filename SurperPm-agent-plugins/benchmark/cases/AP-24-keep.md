---
id: AP-24
title: Alert ownership acknowledgement
level: AP
track: B
...
oracle: |
  L2+ application task: Add owner acknowledgement metadata for alerts, surface acknowledged/unacknowledged filters in the UI/API, and preserve existing alert routing behavior.
...
# AP-24 Alert ownership acknowledgement

## Goal

在 `Alert management` 应用仓中完成一个 L2+ 级别的跨模块需求：Add owner acknowledgement metadata for alerts, surface acknowledged/unacknowledged filters in the UI/API, and preserve existing alert routing behavior.
