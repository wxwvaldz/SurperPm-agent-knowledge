---
id: AP-19
title: Content review escalation state
level: AP
track: B
target_repo_type: node-react-app
target_repo: ap-strapi
...
oracle: |
  L2+ application task: Add an escalation state for content entries that appears in admin list/detail views and API responses while preserving existing draft/publish behavior.
...
# AP-19 Content review escalation state

## Goal

在 `Headless CMS` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an escalation state for content entries that appears in admin list/detail views and API responses while preserving existing draft/publish behavior.
