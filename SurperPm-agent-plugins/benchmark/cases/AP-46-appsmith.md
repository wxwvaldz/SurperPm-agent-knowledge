---
id: AP-46
title: Internal app deployment approval
level: AP
track: B
target_repo_type: java-react-app
target_repo: ap-appsmith
...
oracle: |
  L2+ application task: Add deployment approval metadata for internal apps, carry it through server/API/editor surfaces, and prevent unapproved publish where the repo supports it.
...
# AP-46 Internal app deployment approval

## Goal

在 `Internal tools` 应用仓中完成一个 L2+ 级别的跨模块需求：Add deployment approval metadata for internal apps, carry it through server/API/editor surfaces, and prevent unapproved publish where the repo supports it.
