---
id: AP-13
title: Document signing risk review
level: AP
track: B
target_repo_type: next-app
target_repo: ap-documenso
...
oracle: |
  L2+ application task: Add a risk-review state to documents before sending/signing, propagate it through document list/detail UI and API validation, and avoid changing signing cryptography.
...
# AP-13 Document signing risk review

## Goal

在 `Document signing` 应用仓中完成一个 L2+ 级别的跨模块需求：Add a risk-review state to documents before sending/signing, propagate it through document list/detail UI and API validation, and avoid changing signing cryptography.
