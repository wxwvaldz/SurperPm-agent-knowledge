---
id: AP-50
title: Secret rotation review workflow
level: AP
track: B
target_repo_type: node-react-app
target_repo: ap-infisical
...
oracle: |
  L2+ application task: Add rotation-review metadata for secrets or projects, surface overdue review state in API/dashboard UI, and avoid exposing secret values in logs or tests.
...
# AP-50 Secret rotation review workflow

## Goal

在 `Secret management` 应用仓中完成一个 L2+ 级别的跨模块需求：Add rotation-review metadata for secrets or projects, surface overdue review state in API/dashboard UI, and avoid exposing secret values in logs or tests.
