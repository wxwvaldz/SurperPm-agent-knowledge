---
id: AP-06
title: File expiration and restore affordance
level: AP
track: B
target_repo_type: go-vue-app
target_repo: ap-filebrowser
...
oracle: |
  L2+ application task: Add an optional expiration/retention marker for files or shares, expose it in the API/UI list, and add a restore/clear affordance while respecting the project's maintenance-only scope.
...
# AP-06 File expiration and restore affordance

## Goal

在 `File manager` 应用仓中完成一个 L2+ 级别的跨模块需求：Add an optional expiration/retention marker for files or shares, expose it in the API/UI list, and add a restore/clear affordance while respecting the project's maintenance-only scope.
