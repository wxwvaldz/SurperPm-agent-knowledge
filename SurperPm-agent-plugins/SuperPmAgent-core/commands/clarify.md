...
- Normalizing URL-like, document, or shared-browser external source inputs before they are reflected into the session.
...
- Feishu doc link -> `SuperPmAgent-io` skill: `input/normalize-feishu-doc`
- Bilibili link -> `SuperPmAgent-io` skill: `input/normalize-bilibili-video`
- Douyin link -> `SuperPmAgent-io` skill: `input/normalize-douyin-video`
- Unknown URL -> `SuperPmAgent-io` skill: `input/normalize-url`

Shared browser rules:

- `/clarify` must not treat a raw link as requirement fact.
- `/clarify` must not mark a session ready for `/goal` just because a video or
  document link exists.
- `/clarify` must ask which parts of the external material the PM wants to
  reference.
- If external material and PM explanation conflict, use the PM's later
  confirmation as the authoritative session input.
- Only stable clarified meaning may be written into `notes.md` and
  `decisions.md`.

## Contradictory constraints

When the request pairs **durable per-user or shared state** with **no backend/API/schema changes**, treat it as a scope conflict—not a missing detail.

1. State the conflict in plain language (why frontend-only cannot provide real idempotency).
2. Offer scoped options (prototype vs authoritative server path) with honest tradeoffs.
3. **Do not** mark `ready_for_goal: yes` until `decisions.md` records the PM choice.
4. If the PM has not decided, keep `Open Questions` populated and set `ready_for_goal: no`.
